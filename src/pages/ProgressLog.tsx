...
const changelog: { date: string; items: string[] }[] = [
  {
    date: "2026-06-01",
    items: [
      "Tasks live in the Studio (Phase 3, part 2) — the work that hangs off the CRM. A task always belongs to a client and can optionally be pinned to a specific contact at that client (if that person is later removed, the task survives and just rolls back up to the company). Each task carries a status (To do / In progress / Done), a priority, optional due date, and notes. Two ways in: a cross-client Tasks page with Open / To do / In progress / Done / All filters, one-click complete, and overdue dates flagged; and a Tasks panel on each client's detail screen, scoped to that client with its contacts in the picker. Admin-only behind the same is_admin() RLS, and tasks cascade-delete with their client. This is why contacts came first — a task needs someone to be about.",
