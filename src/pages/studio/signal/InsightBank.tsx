...
// The Insight Bank tab: the source material. Add/edit findings, mark them anonymised,
// and generate LinkedIn drafts from the anonymised ones. All mutation goes through the
// parent so the shell's stat tiles stay in sync.
export function InsightBank({
  insights,
  busyId,
  genId,
  onAdd,
  ...
  busyId: string | null;
  genId: string | null;
  onAdd: (draft: InsightDraft) => void;
  onEditSave: (id: string, draft: InsightDraft) => void;
  onDelete: (id: string) => void;
  onGenerate: (insight: ContentInsight, variantCount: number, ctaType: PostCtaType) => void;
}) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between gap-4">
        <h2 className="num text-xs uppercase tracking-wide text-muted">Insight bank</h2>
        {!adding && (
          <button
            onClick={() => {
              setAdding(true);
